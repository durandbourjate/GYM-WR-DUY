/**
 * Upload-Script: Einrichtungsprüfung → Google Sheets Backend
 *
 * Liest die Einrichtungsfragen und -config aus den TS-Dateien (inline definiert)
 * und sendet sie einzeln an die Apps Script API.
 *
 * Verwendung:
 *   node scripts/uploadEinrichtung.mjs
 */
...
// --- APPS_SCRIPT_URL aus .env.local lesen ---
const envPath = resolve(__dirname, '..', '.env.local')
const envContent = readFileSync(envPath, 'utf-8')
const urlMatch = envContent.match(/VITE_APPS_SCRIPT_URL=(.+)/)
const APPS_SCRIPT_URL = urlMatch?.[1]?.trim()

if (!APPS_SCRIPT_URL) {
  console.error('VITE_APPS_SCRIPT_URL nicht in .env.local gefunden!')
  process.exit(1)
}
...
async function post(action, payload) {
  ...
  try {
    ...
    console.error('Antwort ist kein JSON:', text.slice(0, 200))
    return { error: text.slice(0, 200) }
  }
}

// ═══════════════════════════════════════════════════════
// Fragen-Definitionen (identisch zu einrichtungsFragen.ts)
// ═══════════════════════════════════════════════════════
...
const fragen = [
  ...
  base('einr-tk-kasse', 'tkonto', 'BWL', 2, {
    ...
    konten: [{
      id: 'tk-1000', kontonummer: '1000',
      anfangsbestand: 100, anfangsbestandVorgegeben: true,
      eintraege: [{ seite: 'haben', gegenkonto: '4200', betrag: 30, buchungstext: 'Einkauf bar' }],
      saldo: { betrag: 70, seite: 'soll' },
    }],
    kontenauswahl: { modus: 'eingeschraenkt', konten: ['1000', '1020', '2000', '4200', '5200'] },
    bewertungsoptionen: {
      beschriftungSollHaben: false, kontenkategorie: false, zunahmeAbnahme: false,
      buchungenKorrekt: true, saldoKorrekt: false,
    },
  }),
  ...
  base('einr-bilanz-einfach', 'bilanzstruktur', 'BWL', 2, {
    ...
    bewertungsoptionen: {
      seitenbeschriftung: false, gruppenbildung: false, gruppenreihenfolge: false,
      kontenreihenfolge: false, betraegeKorrekt: true, zwischentotale: false,
...
async function main() {
  console.log(`\n🚀 Upload Einrichtungsprüfung → Backend`)
  ...
  console.log(`\n  Fragen: ${erfolg} ✅ / ${fehler} ❌`)

  // 2. Config hochladen
  process.stdout.write(`\n  📋 Config "${config.id}"... `)
  const configResult = await post('speichereConfig', { config })
  if (configResult.success) {
    console.log('✅')
  } else {
    console.log(`❌ ${configResult.error || 'unbekannter Fehler'}`)
  }

  console.log(`\n✨ Fertig! Prüfung ist im Backend verfügbar.\n`)
}
